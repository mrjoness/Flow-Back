...
parser.add_argument('--diff_type', default='xt_mask', type=str, help='Find vt by subtracting noised or unnoised')
parser.add_argument('--seed', default=13, type=None, help='Random seed for mmseqs clustering')
...
job_dir = f'./jobs/time-batch_adamW_{system}-fix_{loss_type}_m-{mdim}_clamp-{clamp}_attn-{attnevery}_dim-{dim}_nn-{num_nearest_neighbors}_depth-{depth}_eps-{n_epochs}_sigma-{sigma}_batch-{batch_size}_CG-noise-{Ca_std}_lr-{lr}_wdecay-{wdecay}_CGadj-{CGadj}_pos-{pos}_bpack-{batch_pack}_lrdecay-{lrdecay}_diff-{diff_type}'
...
for epoch in range(n_epochs):
    ...
    for i, (x1, x0, res_feats, atom_feats, adj_mat, mask) in tqdm(enumerate(train_loader)):
        ...
        # whether or not to include the adjacency matrix
    ...
    if epoch%eval_every==0 and epoch>0:
        ...
        # can iterate over this and test one at a ti me
