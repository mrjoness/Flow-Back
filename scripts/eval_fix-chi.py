...
parser.add_argument('--solver', default='euler_chi', type=str, help='Which type of ODE solver to use')
...
parser.add_argument('--t_flip', default=0.2, type=float,  help='Intermediate time to check for and correct D-residues')
parser.add_argument('--keep_flip', action='store_true', help='If chirality should be checked and flipped at every time after t_flip')
...
if solver == 'euler':
    save_prefix = f'{save_dir}/{model_path.split("/")[-1]}_ckp-{ckp}_noise-{CG_noise}_nsteps-{nsteps}/'
elif solver == 'euler_chi':
    save_prefix = f'{save_dir}/{model_path.split("/")[-1]}_ckp-{ckp}_noise-{CG_noise}_tflip-{t_flip}_type-{type_flip}/'
elif solver == 'adapt':
    save_prefix = f'{save_dir}/{model_path.split("/")[-1]}_ckp-{ckp}_noise-{CG_noise}_tol-{tol}/'

if keep_flip:
    save_prefix = save_prefix[:-1] + '_keep-flip/'
if mask_prior:
